RSpec.describe Mutest::Reporter::Sequence do
  let(:object)     { described_class.new([reporter_a, reporter_b]) }
  let(:value)      { instance_double(Object)                       }
  let(:reporter_a) { instance_double(Mutest::Reporter, delay: 1.0) }
  let(:reporter_b) { instance_double(Mutest::Reporter, delay: 2.0) }
