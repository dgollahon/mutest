RSpec.describe Mutest::Reporter::CLI::Printer do
  ...
  let(:reportable) { instance_double(Mutest::Result::Env, success?: success?) }
  let(:tty?)       { true                                                     }
  let(:success?)   { true                                                     }
  ...
  describe '#status' do
    ...
    context 'on tty' do
      context 'on success' do
        it_reports Mutest::Color::GREEN.format('foo bar') << "\n"
      end
  ...
  describe '#colorize' do
    let(:class_under_test) do
      Class.new(described_class) do
        def run
          puts(colorize(Mutest::Color::RED, 'foo'))
        end
      end
    end

    context 'when output is a tty?' do
      it_reports Mutest::Color::RED.format('foo') << "\n"
    end
