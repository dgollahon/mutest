RSpec.describe Mutest::Parallel::Master do
  ...
  let(:job_a)                { Mutest::Parallel::Job.new(index: 0, payload: job_payload_a)                }
  let(:job_b)                { Mutest::Parallel::Job.new(index: 1, payload: job_payload_b)                }
  let(:job_result_a)         { Mutest::Parallel::JobResult.new(job: job_a, payload: job_result_payload_a) }
  let(:job_result_b)         { Mutest::Parallel::JobResult.new(job: job_b, payload: job_result_payload_b) }
  ...
  let(:config) do
    Mutest::Parallel::Config.new(
      ...
      processor: processor,
      sink:      sink,
      source:    Mutest::Parallel::Source::Array.new([job_payload_a, job_payload_b])
    )
  end
  ...
  describe '.call' do
    ...
    context 'requesting status' do
      context 'when jobs are done' do
        let(:expected_status)  { Mutest::Parallel::Status.new(payload: 2, active_jobs: Set.new, done: true) }
        let(:expected_results) { [job_result_payload_a, job_result_payload_b] }
      ...
      context 'just after scheduler stops' do
        let(:expected_status)  { Mutest::Parallel::Status.new(payload: 1, active_jobs: [].to_set, done: true) }
        let(:expected_results) { [job_result_payload_a] }
      ...
      context 'when jobs are active' do
        let(:expected_status)  { Mutest::Parallel::Status.new(payload: 1, active_jobs: [job_b].to_set, done: false) }
        let(:expected_results) { [job_result_payload_a, job_result_payload_b] }
      ...
      context 'before jobs are done' do
        let(:expected_status)  { Mutest::Parallel::Status.new(payload: 0, active_jobs: Set.new, done: false) }
        let(:expected_results) { [] }
    ...
    context 'unhandled message received' do
      ...
      it 'raises message' do
        expect { subject }.to raise_error(Mutest::Actor::ProtocolError, 'Unexpected message: :foo')
      end
    end
  end
end
